...
def train_model(data):
    ...
    model.save("task_scheduler_model")
...
def get_suggestions(data, model_path="task_scheduler_model"):
    tasks = data if isinstance(data, list) else data.get("tasks", [])
    if not tasks:
        raise ValueError("No tasks provided for suggestion generation.")
...
def get_weekly_suggestions(task_data):
    ...
    return [[day, suggestion] for day, suggestion in enumerate(suggestions)]
